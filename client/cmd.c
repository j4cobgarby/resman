...
int subcmd_check(int argc UNUSED, char **argv UNUSED) { /*{{{*/
    ...
    printf(head_fmt, "uuid", "type", "user", "time submitted", "info");
    printf(head_fmt, "---", "---", "---", "---", "---");
    ...
    for (int i = 0; i < (int)resp_header->resp_count; i++) {
        ...
        strftime(s_submitted, sizeof(s_submitted), "%a %e %b %T",
                 localtime(&jobs[i].t_submitted));

        if (resp_header->currently_running && i == 0) printf(">");
        else printf(" ");

        printf(tab_fmt, jobs[i].job_uuid, jobtype_lbl[jobs[i].job_type],
               pwd ? pwd->pw_name : "---", s_submitted, jobs[i].msg);
